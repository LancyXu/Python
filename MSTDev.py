...
def mst_algo(locs,dist):
    ...
    mst = [dist_list[0][0]]
    set_list=[set(dist_list[0][0])]
    for path in dist_list[1:]:
        if len(set_list)<len(locs): 
            test=0
            key=path[0]
            loc1=key[0]
            loc2=key[1]
            ...
            for eachset in set_list:
                if loc1 in eachset and loc2 in eachset:
                    test=1
            if test!=1:
                mst.append(key)
                for eachset in set_list:
                    if loc1 not in eachset and loc2 not in eachset:
                        test+=1
                if test==len(set_list):
                    set_list.append(temp_set)
                else:
                    for eachset in set_list:
                        if loc1 in eachset or loc2 in eachset:
                            eachset.add(loc1)
                            eachset.add(loc2)
                            preunion.append(eachset)
                
                 
            if len(preunion)==2:
                new=preunion[0].union(preunion[1])
                set_list.append(new)
                set_list.remove(preunion[0])
                set_list.remove(preunion[1])

    #mst=[(i) for i in set(map(tuple, mst))]
