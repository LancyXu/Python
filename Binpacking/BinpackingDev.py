...
def binpack(articles,bin_cap):
   ...
   """
    ...
    volume=[v for v in articles.values()]
    articles_list=[]
    for key, values in articles.iteritems():
        pair=[key,values]
        articles_list.append(pair)
    articles_list_d=sorted(articles_list, key=lambda x: -x[1])

    load=0.0
    bins=[]
    count=0
    overload=0
    while count<len(volume):
        for each in articles_list_d:
            id=each[0]
            load+=articles[id]
            if load<=bin_cap:
                bins.append(id)
                count+=1
                articles_list_d=articles_list_d[1:]
            else:
                overload+=1
                if overload==1:
                    load=load-articles[id]
                    l_small=0
                    articles_list_d=list(reversed(articles_list_d))
                    for small in articles_list_d:
                        id_small=small[0]
                        load+=articles[id_small]
                        if load<=bin_cap:
                            bins.append(id_small)
                            count+=1
                            l_small+=1
                    articles_list_d=articles_list_d[l_small:]
                    articles_list_d=list(reversed(articles_list_d))
        bin_contents.append(bins)
        bins=[]
        load=0.0
        overload=0
