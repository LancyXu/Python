...
def binpack(articles,bin_cap):
   ...
   """
    import heapq
    my_team_number_or_name = "Sec1Team4"    # always return this variable as the first item
    ...
    for key, values in articles.iteritems():
        pair_reversed=[-values,key]
        list_reversed.append(pair_reversed)
    
    heapq.heapify(list_reversed)
    first=heapq.heappop(list_reversed)
    bins=[[-first[0],first[1]]]
    count=1
    while list_reversed:
        each=heapq.heappop(list_reversed)
        id=each[1]
        indicator=0
        i=0
        while indicator==0:
            abin=bins[i]
            load=abin[0]
            if load+articles[id]<=bin_cap:
                abin.append(id)
                abin[0]=load+articles[id]
                count+=1
                indicator=1
            else:
                i+=1
...
for problem_id in problems[0:1]:
