...
def load_knapsack(things,knapsack_cap):
   ...
   """
    ...
    my_team_number_or_name = "lxu08"    # always return this variable as the first item
    ...
    things_list=[]
    for key, values in things.iteritems():
        ratio=float(values[1]/values[0])
        pair=[key,[values[0],values[1],ratio]]
        things_list.append(pair)
    aftersort=[]
    aftersort=sorted(things_list, key=lambda x: (x[1][2],x[1][1],-x[1][0]), reverse=True)
    for each in aftersort:
        pack_item = each[0]
        load += things[pack_item][0]
        if load <= knapsack_cap:
            items_to_pack.append(pack_item)
            items_to_pack_temp=items_to_pack[0:len(items_to_pack)-1]
            value += things[pack_item][1]
            value0=value-things[pack_item][1]
            load0=load-things[pack_item][0]
    
    rest=aftersort[len(items_to_pack):]
    rest=sorted(rest,key=lambda x:(-x[1][1],x[1][0]))

    count=0
    for i in rest:
        for j in rest[(rest.index(i)+1):]:
            if count<3:
                value_temp=value0
                load_temp=load0
                pack_item1 = i[0]
                pack_item2 = j[0]
                load_temp += things[pack_item1][0]+things[pack_item2][0]
                if load_temp <= knapsack_cap:
                    value_temp += things[pack_item1][1]+things[pack_item2][1]
                    if value_temp > value:
                        items_to_pack_temp.append(pack_item1)
                        items_to_pack_temp.append(pack_item2)
                        items_to_pack=set(items_to_pack_temp)
                        items_to_pack_temp.remove(pack_item1)
                        items_to_pack_temp.remove(pack_item2)
                        value=value_temp
                        count+=1
    items_to_pack=list(items_to_pack)
