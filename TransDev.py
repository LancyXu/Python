...
def trans(dist, dcs, stores_vol):
    trail_cu_ft = 4000.0
    num_days_year = 365.0
    my_team_or_name = "lxu08"
    result = []
    
    dist_list=[(key,value) for key, value in dist.iteritems()]
    ...
    for path in dist_list:
        store=path[0][1]
        #dc=path[0][0]
        if store not in assigned_store:
            choices=[(i,store,dist[i,store]) for i in dcs.keys()]
            choices=sorted(choices, key=lambda x: x[2])
            count=0
            for choice in choices:
                dc=choice[0]
                if count==0:
                    if num_stores[dc]<dcs[dc][1]:
                        volume_add=store_v_tmp[store]
                        if volume[dc]+volume_add<=dcs[dc][0]:
                            driver_needed=round(volume_add/trail_cu_ft+0.5,0)
                            if drivers[dc]+driver_needed<=dcs[dc][2]:
                                new=(store,dc)
                                result.append(new)
                                num_stores[dc]+=1
                                volume[dc]+=volume_add
                                drivers[dc]+=driver_needed
                                assigned_store.add(store)
                                count=1
